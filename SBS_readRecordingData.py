...
def readSaveFile(file_name):
    """
    Load synchronized raw IQ radar and reference sensor data from a pickle file.

    Args:
        file_name (str | os.PathLike): Path to a .pkl file containing repeated groups of four pickled objects:

            - file_info: Metadata about the measurement.
    ...
    Returns:
        tuple:
            ...
            - file_info (Any): Measurement metadata.
            - radar_var (Any): Radar configuration.
            - synchro_info (Any): Synchronization info.
            - measurement_data (Any): PSG and radar data.
...
def readRadarMakeEpochs(
    ...
    Args:
        ...
        parallelize (bool): Process epochs in parallel with joblib.
        parallel_workers (int): Number of parallel workers.
        parallel_blockLen (int): Number of epochs per parallel batch.
        parallel_verboseLvl (int): Joblib verbosity.
    ...
    Returns:
        tuple:
            ...
            - phaseEpochs (list[np.ndarray]): List of length numEpochs; each shape (radar_adcsamp_num, epochLen_inSamples).
              ...
              Units: millimeters for phaseDemod='arctan'. May be DC-removed and/or normalized.
    ...
    if not parallelize:
        for epochi,currEpochStart in enumerate(epochStartInds):
            ...
            dataCompEpoch = deinterleaveRadarData(rawInputData[currEpochStart:currEpochEnd],TX,RX) # shape: frame,chirp,bin
...
def readFullRefData(measurement_data,sensor_idx,printReadyMsg=False):
    """
    ...
    Args:
        ...
        printReadyMsg (bool): If True, prints a confirmation message.
